...
const MOCK_PATIENTS = [
  ...
  },
  {
    ...
    riskLevel: 'Critical',
    phases: [
      { phase: 'Prevention', withoutAI: 'Missed kidney screening → Dialysis', withAI: 'Early detection → Prevention' },
      { phase: 'Acute', withoutAI: 'Kidney failure emergency', withAI: 'Controlled progression' },
      { phase: 'Recovery', withoutAI: 'Dialysis dependency', withAI: 'Kidney function preservation' },
      { phase: 'Post-discharge', withoutAI: 'Frequent dialysis visits', withAI: 'Home monitoring + medication' },
    ],
    readmit: { before: 0.45, after: 0.15 },
    timeline: [
      { date: '2024-01-01', event: 'Kidney function screening overdue by 6 months', ai: false, impact: 'Critical care gap' },
      { date: '2024-01-15', event: 'Routine blood work shows elevated creatinine', ai: false, impact: 'Late detection' },
      { date: '2024-02-01', event: 'Kidney failure - emergency dialysis required', ai: false, impact: 'Acute crisis' },
      { date: '2024-02-15', event: 'Dialysis dependency established', ai: false, impact: 'Chronic condition' },
      { date: '2024-03-01', event: 'Frequent dialysis visits - 3x weekly', ai: false, impact: 'High care burden' },
      { date: '2024-03-15', event: 'Infection from dialysis access - readmission', ai: false, impact: 'Complication' },
      { date: '2024-01-01', event: 'AI flags kidney screening gap - urgent intervention', ai: true, impact: 'Early detection' },
      { date: '2024-01-10', event: 'Kidney function test completed - early nephropathy', ai: true, impact: 'Timely diagnosis' },
      { date: '2024-01-20', event: 'Medication adjustment - ACE inhibitor optimization', ai: true, impact: 'Preventive treatment' },
      { date: '2024-02-01', event: 'Kidney function stabilized - no dialysis needed', ai: true, impact: 'Crisis averted' },
      { date: '2024-02-15', event: 'Home monitoring system installed', ai: true, impact: 'Continuous care' },
      { date: '2024-03-01', event: 'Kidney function improving - medication effective', ai: true, impact: 'Positive outcome' },
...
function CareCostGapsCenterAIDialog({ open, onClose }) {
  ...
  for (let i = 1; i <= 35; i++) {
    ...
    // Special age for Lisa Rodriguez (3rd patient, index 2)
    const age = patientName === 'Lisa Rodriguez' ? 72 : 45 + (i % 35);
    ...
    patients.push({
      ...
      totalSavings: timelineEvents.filter(e => e.ai).reduce((sum, e) => sum + (e.costSaved || 0), 0),
  ...
  const generateTimelineEvent = (driverId, eventIndex, patientIndex, isAI, date) => {
    ...
    if (driverId === 'chronicDisease' && currentPatientName === 'Lisa Rodriguez') {
      const lisaTimeline = [
        ...
        { type: 'No Fill Detection', description: 'Patient does not fill the prescription. This is identified from pharmacy records or payer claims history', ai: true, costSaved: 2500, agent: 'Chronic-Agent-001' },
      ...
      const baseData = {
        chronicDisease: {
          patients: [
            ...
            { id: 'P002', name: 'Michael Chen', age: 72, conditions: ['CHF', 'COPD'], riskScore: 92, lastVisit: '2024-01-10', gaps: ['No Weight Monitoring', 'Missing Spirometry'] },
                ...
                {selectedTimelinePatient && timelineView === 'patient-detail' ? (
                  <Box>
                    ...
                    <Grid container spacing={2}>
                              ...
                              <Box key={event.id} sx={{ 
                                ...
                                <Box sx={{ ml: 2, flex: 1 }}>
                                  <Card sx={{ 
                                    ...
                                    {/* Proof Button for Lisa Rodriguez and Jennifer Williams */}
                                    {event.ai && event.proofData && (selectedTimelinePatient?.name === 'Lisa Rodriguez' || selectedTimelinePatient?.name === 'Jennifer Williams') && (
                                      <Box sx={{ mt: 2 }}>
                                        <Button
                                          variant="outlined"
                                          size="small"
                      ...
                      {/* Outcome Metrics for Lisa Rodriguez */}
                      {selectedTimelinePatient.name === 'Lisa Rodriguez' && (
                        <Grid item xs={12}>
